...
func TestSecurityProfilesOperatorXK8SIoSecurityProfileNodeStatusV1Alpha1Resource(t *testing.T) {
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_security_profiles_operator_x_k8s_io_security_profile_node_status_v1alpha1", key))
	}
}
