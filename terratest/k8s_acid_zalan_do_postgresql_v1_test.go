...
func TestAcidZalanDoPostgresqlV1Resource(t *testing.T) {
	path := "../examples/resources/k8s_acid_zalan_do_postgresql_v1"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_acid_zalan_do_postgresql_v1", key))
	}
}
