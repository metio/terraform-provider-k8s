...
func TestCamelApacheOrgBuildV1Resource(t *testing.T) {
	path := "../examples/resources/k8s_camel_apache_org_build_v1"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_camel_apache_org_build_v1", key))
	}
}
