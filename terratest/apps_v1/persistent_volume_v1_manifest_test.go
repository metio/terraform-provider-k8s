...
func TestPersistentVolumeV1Manifest(t *testing.T) {
	path := "../../examples/data-sources/k8s_persistent_volume_v1_manifest"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("data %s.%s did not produce an output", "k8s_persistent_volume_v1_manifest", key))
	}
}
