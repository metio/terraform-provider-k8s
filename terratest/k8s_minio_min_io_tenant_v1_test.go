...
func TestMinioMinIoTenantV1Resource(t *testing.T) {
	path := "../examples/resources/k8s_minio_min_io_tenant_v1"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_minio_min_io_tenant_v1", key))
	}
}
