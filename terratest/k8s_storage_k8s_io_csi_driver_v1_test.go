...
func TestStorageK8SIoCSIDriverV1Resource(t *testing.T) {
	path := "../examples/resources/k8s_storage_k8s_io_csi_driver_v1"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_storage_k8s_io_csi_driver_v1", key))
	}
}
