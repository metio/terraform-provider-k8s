...
func TestPipesServicesK8SAwsPipeV1Alpha1Manifest(t *testing.T) {
	path := "../../examples/data-sources/k8s_pipes_services_k8s_aws_pipe_v1alpha1_manifest"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("data %s.%s did not produce an output", "k8s_pipes_services_k8s_aws_pipe_v1alpha1_manifest", key))
	}
}
