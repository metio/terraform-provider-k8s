...
func TestKyvernoIoCleanupPolicyV2Manifest(t *testing.T) {
	path := "../../examples/data-sources/k8s_kyverno_io_cleanup_policy_v2_manifest"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("data %s.%s did not produce an output", "k8s_kyverno_io_cleanup_policy_v2_manifest", key))
	}
}
