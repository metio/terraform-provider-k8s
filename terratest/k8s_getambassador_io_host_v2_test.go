...
func TestGetambassadorIoHostV2Resource(t *testing.T) {
	path := "../examples/resources/k8s_getambassador_io_host_v2"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_getambassador_io_host_v2", key))
	}
}
