...
func TestLonghornIoBackupV1Beta1Resource(t *testing.T) {
	path := "../examples/resources/k8s_longhorn_io_backup_v1beta1"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_longhorn_io_backup_v1beta1", key))
	}
}
