...
func TestFlaggerAppCanaryV1Beta1Resource(t *testing.T) {
	path := "../examples/resources/k8s_flagger_app_canary_v1beta1"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_flagger_app_canary_v1beta1", key))
	}
}
