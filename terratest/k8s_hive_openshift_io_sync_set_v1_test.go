...
func TestHiveOpenshiftIoSyncSetV1Resource(t *testing.T) {
	path := "../examples/resources/k8s_hive_openshift_io_sync_set_v1"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_hive_openshift_io_sync_set_v1", key))
	}
}
