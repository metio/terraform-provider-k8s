...
func TestCiliumIoCiliumIdentityV2Manifest(t *testing.T) {
	path := "../../examples/data-sources/k8s_cilium_io_cilium_identity_v2_manifest"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("data %s.%s did not produce an output", "k8s_cilium_io_cilium_identity_v2_manifest", key))
	}
}
