...
func TestAppsKubedlIoCronV1Alpha1Resource(t *testing.T) {
	path := "../examples/resources/k8s_apps_kubedl_io_cron_v1alpha1"
	...
	for key, value := range outputMap {
		assert.NotEmpty(t, value, fmt.Sprintf("resource %s.%s did not produce an output", "k8s_apps_kubedl_io_cron_v1alpha1", key))
	}
}
