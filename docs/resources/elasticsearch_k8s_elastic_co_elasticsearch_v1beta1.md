...
page_title: "k8s_elasticsearch_k8s_elastic_co_elasticsearch_v1beta1 Resource - terraform-provider-k8s"
subcategory: "elasticsearch.k8s.elastic.co/v1beta1"
description: |-
  Elasticsearch represents an Elasticsearch resource in a Kubernetes cluster.
---

# k8s_elasticsearch_k8s_elastic_co_elasticsearch_v1beta1 (Resource)
...
- `grpc` (Attributes) GRPC specifies an action involving a GRPC port. This is a beta field and requires enabling GRPCContainerProbe feature gate. (see [below for nested schema](#nestedatt--spec--node_sets--pod_template--spec--containers--working_dir--grpc))
...
- `failure_threshold` (Number) Minimum consecutive failures for the probe to be considered failed after having succeeded. Defaults to 3. Minimum value is 1.
- `grpc` (Attributes) GRPC specifies an action involving a GRPC port. This is a beta field and requires enabling GRPCContainerProbe feature gate. (see [below for nested schema](#nestedatt--spec--node_sets--pod_template--spec--ephemeral_containers--working_dir--grpc))
...
- `failure_threshold` (Number) Minimum consecutive failures for the probe to be considered failed after having succeeded. Defaults to 3. Minimum value is 1.
- `grpc` (Attributes) GRPC specifies an action involving a GRPC port. This is a beta field and requires enabling GRPCContainerProbe feature gate. (see [below for nested schema](#nestedatt--spec--node_sets--pod_template--spec--init_containers--working_dir--grpc))
...
- `max_unavailable` (Dynamic) An eviction is allowed if at most 'maxUnavailable' pods selected by 'selector' are unavailable after the eviction, i.e. even in absence of the evicted pod. For example, one can prevent all voluntary evictions by specifying 0. This is a mutually exclusive setting with 'minAvailable'.
...
- `match_expressions` (Attributes List) matchExpressions is a list of label selector requirements. The requirements are ANDed. (see [below for nested schema](#nestedatt--spec--pod_disruption_budget--spec--selector--match_expressions))
- `match_labels` (Map of String) matchLabels is a map of {key,value} pairs. A single {key,value} in the matchLabels map is equivalent to an element of matchExpressions, whose key field is 'key', the operator is 'In', and the values array contains only 'value'. The requirements are ANDed.

<a id="nestedatt--spec--pod_disruption_budget--spec--selector--match_expressions"></a>
### Nested Schema for `spec.pod_disruption_budget.spec.selector.match_expressions`
