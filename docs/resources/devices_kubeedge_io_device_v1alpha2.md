...
```terraform
resource "k8s_devices_kubeedge_io_device_v1alpha2" "minimal" {
  metadata = {
    name = "test"
  }
}
...
- `customized_values` (Dynamic) Customized values for visitor of provided protocols
...
- `config_data` (Dynamic) Required: The configData of customized protocol
...
- `customized_values` (Dynamic) Customized values for provided protocol
...
- `config_data` (Dynamic) Any config data
