...
page_title: "k8s_helm_toolkit_fluxcd_io_helm_release_v2beta1 Resource - terraform-provider-k8s"
subcategory: "helm.toolkit.fluxcd.io/v2beta1"
description: |-
  HelmRelease is the Schema for the helmreleases API
---

# k8s_helm_toolkit_fluxcd_io_helm_release_v2beta1 (Resource)
...
- `secret_ref` (Attributes) SecretRef holds the name to a secret that contains a key with the kubeconfig file as the value. If no key is specified the key will default to 'value'. The secret must be in the same namespace as the HelmRelease. It is recommended that the kubeconfig is self-contained, and the secret is regularly updated if credentials such as a cloud-access-token expire. Cloud specific 'cmd-path' auth helpers will not function without adding binaries and credentials to the Pod that is responsible for reconciling the HelmRelease. (see [below for nested schema](#nestedatt--spec--kube_config--secret_ref))
