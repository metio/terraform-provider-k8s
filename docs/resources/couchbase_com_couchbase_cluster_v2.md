...
- `ldap` (Attributes) LDAP Settings (see [below for nested schema](#nestedatt--spec--security--ldap))
- `rbac` (Attributes) Couchbase RBAC Users (see [below for nested schema](#nestedatt--spec--security--rbac))
...
- `bind_secret` (String) BindSecret is the name of a Kubernetes secret to use containing password for LDAP user binding
- `hosts` (List of String) List of LDAP hosts.
- `port` (Number) LDAP port. This is typically 389 for LDAP, and 636 for LDAPS.

Optional:

- `authentication_enabled` (Boolean) Enables using LDAP to authenticate users.
- `authorization_enabled` (Boolean) Enables use of LDAP groups for authorization.
...
- `cacert` (String) CA Certificate in PEM format to be used in LDAP server certificate validation
...
- `encryption` (String) Encryption method to communicate with LDAP servers. Can be StartTLSExtension, TLS, or false.
...
- `tls_secret` (String) TLSSecret is the name of a Kubernetes secret to use for LDAP ca cert.
...
### Nested Schema for `spec.servers.pod.spec.topology_spread_constraints.label_selector.match_labels`
...
- `object_endpoint` (Attributes) ObjectEndpoint contains the configuration for connecting to a custom S3 compliant object store. (see [below for nested schema](#nestedatt--spec--backup--object_endpoint))
...
- `s3_secret` (String) S3Secret contains the region and credentials for operating backups in S3. This field must be popluated when the 'spec.s3bucket' field is specified for a backup or restore resource.
...
- `use_iam_role` (Boolean) UseIAMRole enables backup to fetch EC2 instance metadata. This allows the AWS SDK to use the EC2's IAM Role for S3 access. UseIAMRole will ignore credentials in s3Secret.
...
- `secret` (String) The name of the secret, in this namespace, that contains the CA certificate for verification of a TLS endpoint (when required, e.g. not signed by a public CA). The secret must have the key with the name 'tls.crt'
- `url` (String) The host/address of the custom object endpoint.
- `use_virtual_path` (Boolean) UseVirtualPath will force the AWS SDK to use the new virtual style paths. by default alternative path style URLs which are often required by S3 compatible object stores.
...
- `auto_failover_server_group` (Boolean) AutoFailoverServerGroup whether to enable failing over a server group.
...
- `server_secret_name` (String) ServerSecretName specifies the secret name, in the same namespace as the cluster, that contains server TLS data.  The secret is expected to contain 'tls.crt' and 'tls.key' as per the kubernetes.io/tls secret type.  It may also contain 'ca.crt'.
...
- `server_secret` (String) ServerSecret is a secret name containing TLS certs used by each Couchbase member pod for the communication between Couchbase server and its clients.  The secret must contain a certificate chain (data key 'couchbase-operator.crt') and a private key (data key 'couchbase-operator.key').  The private key must be in the PKCS#1 RSA format.  The certificate chain must have a required set of X.509v3 subject alternative names for all cluster addressing modes.  See the Operator TLS documentation for more information.
...
- `hostname` (String) Hostname is the connection string to use to connect the remote cluster.
