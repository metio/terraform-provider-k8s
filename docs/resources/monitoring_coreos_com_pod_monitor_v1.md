...
- `attach_metadata` (Attributes) Attaches node metadata to discovered targets. Only valid for role: pod. Only valid in Prometheus versions 2.35.0 and newer. (see [below for nested schema](#nestedatt--spec--attach_metadata))
...
- `target_port` (Dynamic) Deprecated: Use 'port' instead.
...
- `credentials` (Attributes) The secret's key that contains the credentials of the request (see [below for nested schema](#nestedatt--spec--pod_metrics_endpoints--authorization--credentials))
...
- `ca` (Attributes) Struct containing the CA cert to use for the targets. (see [below for nested schema](#nestedatt--spec--pod_metrics_endpoints--tls_config--ca))
