...
page_title: "k8s_camel_apache_org_integration_platform_v1 Resource - terraform-provider-k8s"
...
# k8s_camel_apache_org_integration_platform_v1 (Resource)
...
```terraform
resource "k8s_camel_apache_org_integration_platform_v1" "minimal" {
  metadata = {
    name = "test"
  }
}
...
- `configuration` (Attributes List) list of configuration properties to be attached to all the Integration/IntegrationKits built from this IntegrationPlatform (see [below for nested schema](#nestedatt--spec--configuration))
...
- `resources` (Map of String) Deprecated: not used
...
- `build_strategy` (String) the strategy to adopt for building an Integration base image
- `kaniko_build_cache` (Boolean) Deprecated: Use PublishStrategyOptions instead enables Kaniko publish strategy cache
...
- `persistent_volume_claim` (String) Deprecated: Use PublishStrategyOptions instead the Persistent Volume Claim used by Kaniko publish strategy, if cache is enabled
- `publish_strategy` (String) the strategy to adopt for publishing an Integration base image
- `publish_strategy_options` (Map of String)
...
<a id="nestedatt--spec--build--maven--ca_secret"></a>
