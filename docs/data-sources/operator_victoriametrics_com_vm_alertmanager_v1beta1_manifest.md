...
- `storage` (Attributes) Storage is the definition of how storage will be used by the VMAlertmanager instances. (see [below for nested schema](#nestedatt--spec--storage))
- `templates` (Attributes List) Templates is a list of ConfigMap key references for ConfigMaps in the same namespace as the VMAlertmanager object, which shall be mounted into the VMAlertmanager Pods. The Templates are mounted into /etc/vm/templates/<configmap-name>/<configmap-key>. (see [below for nested schema](#nestedatt--spec--templates))
