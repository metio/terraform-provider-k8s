...
page_title: "k8s_cluster_x_k8s_io_machine_deployment_v1beta1 Data Source - terraform-provider-k8s"
subcategory: "cluster.x-k8s.io"
description: |-
  MachineDeployment is the Schema for the machinedeployments API.
---

# k8s_cluster_x_k8s_io_machine_deployment_v1beta1 (Data Source)
...
- `rollout_after` (String) RolloutAfter is a field to indicate a rollout should be performed after the specified time even if no changes have been made to the MachineDeployment. Example: In the YAML the time can be specified in the RFC3339 format. To specify the rolloutAfter target as March 9, 2023, at 9 am UTC use '2023-03-09T09:00:00Z'.
- `selector` (Attributes) Label selector for machines. Existing MachineSets whose machines are selected by this will be the ones affected by this deployment. It must match the machine template's labels. (see [below for nested schema](#nestedatt--spec--selector))
- `strategy` (Attributes) The deployment strategy to use to replace existing machines with new ones. (see [below for nested schema](#nestedatt--spec--strategy))
