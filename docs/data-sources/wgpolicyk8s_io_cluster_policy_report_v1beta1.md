...
page_title: "k8s_wgpolicyk8s_io_cluster_policy_report_v1beta1 Data Source - terraform-provider-k8s"
subcategory: "wgpolicyk8s.io"
description: |-
  ClusterPolicyReport is the Schema for the clusterpolicyreports API
---

# k8s_wgpolicyk8s_io_cluster_policy_report_v1beta1 (Data Source)
...
```terraform
data "k8s_wgpolicyk8s_io_cluster_policy_report_v1beta1" "example" {
  metadata = {
    name = "some-name"
...
- `configuration` (Attributes) Configuration is an optional field which can be used to specify a contract between PolicyReport generators and consumers (see [below for nested schema](#nestedatt--configuration))
...
- `source` (String) Source is an identifier for the source e.g. a policy engine that manages this report. Use this field if all the results are produced by a single policy engine. If the results are produced by multiple sources e.g. different engines or scanners, then use the Source field at the PolicyReportResult level.
...
- `limits` (Attributes) (see [below for nested schema](#nestedatt--configuration--limits))

<a id="nestedatt--configuration--limits"></a>
### Nested Schema for `configuration.limits`

Read-Only:

- `max_results` (Number) MaxResults is the maximum number of results contained in the report
- `status_filter` (List of String) StatusFilter indicates that the PolicyReport contains only those reports with statuses specified in this list
...
- `source` (String) Source is an identifier for the policy engine that manages this report If the Source is specified at this level, it will override the Source field set at the PolicyReport level
