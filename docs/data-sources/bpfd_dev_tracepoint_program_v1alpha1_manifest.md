...
page_title: "k8s_bpfd_dev_tracepoint_program_v1alpha1_manifest Data Source - terraform-provider-k8s"
subcategory: "bpfd.dev"
description: |-
  TracepointProgram is the Schema for the TracepointPrograms API
---

# k8s_bpfd_dev_tracepoint_program_v1alpha1_manifest (Data Source)
...
```terraform
data "k8s_bpfd_dev_tracepoint_program_v1alpha1_manifest" "example" {
  metadata = {
    name = "some-name"
...
- `spec` (Attributes) TracepointProgramSpec defines the desired state of TracepointProgram (see [below for nested schema](#nestedatt--spec))
...
- `names` (List of String) Names refers to the names of kernel tracepoints to attach the bpf program to.
...
- `imagepullsecret` (Attributes) ImagePullSecret is the name of the secret bpfd should use to get remote image repository secrets. (see [below for nested schema](#nestedatt--spec--bytecode--image--imagepullsecret))
