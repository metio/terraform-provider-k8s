...
page_title: "k8s_fluentbit_fluent_io_fluent_bit_v1alpha2_manifest Data Source - terraform-provider-k8s"
subcategory: "fluentbit.fluent.io"
description: |-
  FluentBit is the Schema for the fluentbits API
---

# k8s_fluentbit_fluent_io_fluent_bit_v1alpha2_manifest (Data Source)
...
- `spec` (Attributes) FluentBitSpec defines the desired state of FluentBit (see [below for nested schema](#nestedatt--spec))
...
- `command` (List of String) Fluent Bit Watcher command.
- `container_log_real_path` (String) Container log path
- `container_security_context` (Attributes) ContainerSecurityContext holds container-level security attributes. (see [below for nested schema](#nestedatt--spec--container_security_context))
- `disable_log_volumes` (Boolean) DisableLogVolumes removes the hostPath mounts for varlibcontainers, varlogs and systemd.
- `disable_service` (Boolean) DisableService tells if the fluentbit service should be deployed.
- `dns_policy` (String) Set DNS policy for the pod. Defaults to 'ClusterFirst'. Valid values are'ClusterFirstWithHostNet', 'ClusterFirst', 'Default' or 'None'.
- `env_vars` (Attributes List) EnvVars represent environment variables that can be passed to fluentbit pods. (see [below for nested schema](#nestedatt--spec--env_vars))
...
- `init_containers` (Attributes List) InitContainers represents the pod's init containers. (see [below for nested schema](#nestedatt--spec--init_containers))
- `internal_mount_propagation` (String) MountPropagation option for internal mounts
- `labels` (Map of String) Labels to add to each FluentBit pod
- `liveness_probe` (Attributes) LivenessProbe represents the pod's liveness probe. (see [below for nested schema](#nestedatt--spec--liveness_probe))
- `metrics_port` (Number) MetricsPort is the port used by the metrics server. If this option is set, HttpPort from ClusterFluentBitConfig needs to match this value. Default is 2020.
- `namespace_fluent_bit_cfg_selector` (Attributes) NamespacedFluentBitCfgSelector selects the namespace FluentBitConfig associated with this FluentBit (see [below for nested schema](#nestedatt--spec--namespace_fluent_bit_cfg_selector))
...
- `position_db` (Attributes) Storage for position db. You will use it if tail input is enabled. (see [below for nested schema](#nestedatt--spec--position_db))
...
- `readiness_probe` (Attributes) ReadinessProbe represents the pod's readiness probe. (see [below for nested schema](#nestedatt--spec--readiness_probe))
...
- `scheduler_name` (String) SchedulerName represents the desired scheduler for fluent-bit pods.
...
<a id="nestedatt--spec--namespace_fluent_bit_cfg_selector"></a>
### Nested Schema for `spec.namespace_fluent_bit_cfg_selector`

Optional:

- `match_expressions` (Attributes List) matchExpressions is a list of label selector requirements. The requirements are ANDed. (see [below for nested schema](#nestedatt--spec--namespace_fluent_bit_cfg_selector--match_expressions))
- `match_labels` (Map of String) matchLabels is a map of {key,value} pairs. A single {key,value} in the matchLabelsmap is equivalent to an element of matchExpressions, whose key field is 'key', theoperator is 'In', and the values array contains only 'value'. The requirements are ANDed.

<a id="nestedatt--spec--namespace_fluent_bit_cfg_selector--match_expressions"></a>
### Nested Schema for `spec.namespace_fluent_bit_cfg_selector.match_expressions`
