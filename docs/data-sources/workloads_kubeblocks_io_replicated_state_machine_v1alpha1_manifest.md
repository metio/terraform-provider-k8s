...
page_title: "k8s_workloads_kubeblocks_io_replicated_state_machine_v1alpha1_manifest Data Source - terraform-provider-k8s"
subcategory: "workloads.kubeblocks.io"
description: |-
  ReplicatedStateMachine is the Schema for the replicatedstatemachines API.
---

# k8s_workloads_kubeblocks_io_replicated_state_machine_v1alpha1_manifest (Data Source)
...
- `selector` (Attributes) Represents a label query over pods that should match the replica count. It must match the pod template's labels. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors (see [below for nested schema](#nestedatt--spec--selector))
- `service_name` (String) Refers to the name of the service that governs this StatefulSet. This service must exist before the StatefulSet and is responsible for the network identity of the set. Pods get DNS/hostnames that follow a specific pattern.
...
- `paused` (Boolean) Indicates that the rsm is paused, meaning the reconciliation of this rsm object will be paused.
...
- `service` (Attributes) Defines the behavior of a service spec. Provides read-write service. https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status (see [below for nested schema](#nestedatt--spec--service))
- `update_strategy` (Attributes) Indicates the StatefulSetUpdateStrategy that will be employed to update Pods in the RSM when a revision is made to Template. UpdateStrategy.Type will be set to appsv1.OnDeleteStatefulSetStrategyType if MemberUpdateStrategy is not nil (see [below for nested schema](#nestedatt--spec--update_strategy))
- `volume_claim_templates` (Attributes List) Represents a list of claims that pods are allowed to reference. The ReplicatedStateMachine controller is responsible for mapping network identities to claims in a way that maintains the identity of a pod. Every claim in this list must have at least one matching (by name) volumeMount in one container in the template. A claim in this list takes precedence over any volumes in the template, with the same name. (see [below for nested schema](#nestedatt--spec--volume_claim_templates))
...
- `client_ip` (Attributes) clientIP contains the configurations of Client IP based session affinity. (see [below for nested schema](#nestedatt--spec--alternative_services--spec--session_affinity_config--client_ip))

<a id="nestedatt--spec--alternative_services--spec--session_affinity_config--client_ip"></a>
### Nested Schema for `spec.alternative_services.spec.session_affinity_config.client_ip`
...
- `password` (Attributes) Represents the user's password for the credential. The corresponding environment variable will be KB_RSM_PASSWORD. (see [below for nested schema](#nestedatt--spec--credential--password))
- `username` (Attributes) Defines the user's name for the credential. The corresponding environment variable will be KB_RSM_USERNAME. (see [below for nested schema](#nestedatt--spec--credential--username))
...
- `name` (String) Specifies the name of the template. Each instance of the template derives its name from the RSM's Name, the template's Name and the instance's ordinal. The constructed instance name follows the pattern $(rsm.name)-$(template.name)-$(ordinal). The ordinal starts from 0 by default.

Optional:

- `annotations` (Map of String) Defines annotations to override. Add new or override existing annotations.
...
- `image` (String) Defines image to override. Will override the first container's image of the pod.
- `labels` (Map of String) Defines labels to override. Add new or override existing labels.
- `node_name` (String) Defines NodeName to override.
- `node_selector` (Map of String) Defines NodeSelector to override.
- `replicas` (Number) Number of replicas of this template. Default is 1.
- `resources` (Attributes) Defines Resources to override. Will override the first container's resources of the pod. (see [below for nested schema](#nestedatt--spec--instances--resources))
- `runtime_class_name` (String) Defines RuntimeClass to override.
- `tolerations` (Attributes List) Defines Tolerations to override. Add new or override existing tolerations. (see [below for nested schema](#nestedatt--spec--instances--tolerations))
