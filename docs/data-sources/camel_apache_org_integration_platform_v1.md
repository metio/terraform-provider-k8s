...
```terraform
data "k8s_camel_apache_org_integration_platform_v1" "example" {
  metadata = {
    name      = "some-name"
...
- `build_catalog_tool_timeout` (String) the timeout (in seconds) to use when creating the build tools container image
...
<a id="nestedatt--spec--build--build_configuration"></a>
