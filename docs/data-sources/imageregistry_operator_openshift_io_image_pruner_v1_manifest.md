...
page_title: "k8s_imageregistry_operator_openshift_io_image_pruner_v1_manifest Data Source - terraform-provider-k8s"
...
# k8s_imageregistry_operator_openshift_io_image_pruner_v1_manifest (Data Source)
...
```terraform
data "k8s_imageregistry_operator_openshift_io_image_pruner_v1_manifest" "example" {
  metadata = {
    name = "some-name"
...
- `spec` (Attributes) ImagePrunerSpec defines the specs for the running image pruner. (see [below for nested schema](#nestedatt--spec))
...
- `affinity` (Attributes) affinity is a group of node affinity scheduling rules for the image pruner pod. (see [below for nested schema](#nestedatt--spec--affinity))
- `failed_jobs_history_limit` (Number) failedJobsHistoryLimit specifies how many failed image pruner jobs to retain. Defaults to 3 if not set.
- `ignore_invalid_image_references` (Boolean) ignoreInvalidImageReferences indicates whether the pruner can ignore errors while parsing image references.
- `keep_tag_revisions` (Number) keepTagRevisions specifies the number of image revisions for a tag in an image stream that will be preserved. Defaults to 3.
- `keep_younger_than` (Number) keepYoungerThan specifies the minimum age in nanoseconds of an image and its referrers for it to be considered a candidate for pruning. DEPRECATED: This field is deprecated in favor of keepYoungerThanDuration. If both are set, this field is ignored and keepYoungerThanDuration takes precedence.
- `keep_younger_than_duration` (String) keepYoungerThanDuration specifies the minimum age of an image and its referrers for it to be considered a candidate for pruning. Defaults to 60m (60 minutes).
- `log_level` (String) logLevel sets the level of log output for the pruner job.  Valid values are: 'Normal', 'Debug', 'Trace', 'TraceAll'. Defaults to 'Normal'.
- `node_selector` (Map of String) nodeSelector defines the node selection constraints for the image pruner pod.
- `resources` (Attributes) resources defines the resource requests and limits for the image pruner pod. (see [below for nested schema](#nestedatt--spec--resources))
...
- `successful_jobs_history_limit` (Number) successfulJobsHistoryLimit specifies how many successful image pruner jobs to retain. Defaults to 3 if not set.
- `suspend` (Boolean) suspend specifies whether or not to suspend subsequent executions of this cronjob. Defaults to false.
- `tolerations` (Attributes List) tolerations defines the node tolerations for the image pruner pod. (see [below for nested schema](#nestedatt--spec--tolerations))
