	...
	response.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"spec": schema.SingleNestedAttribute{
				...
				Attributes: map[string]schema.Attribute{
					"name": schema.StringAttribute{
						Description:         "The friendly name to assign to the new OU. Regex Pattern: '^[sS]*$'",
						MarkdownDescription: "The friendly name to assign to the new OU. Regex Pattern: '^[sS]*$'",
						Required:            true,
						Optional:            false,
						Computed:            false,
					},
					...
					"parent_id": schema.StringAttribute{
						...
						MarkdownDescription: "The unique identifier (ID) of the parent root or OU that you want to create the new OU in. The regex pattern (http://wikipedia.org/wiki/regex) for a parent ID string requires one of the following: * Root - A string that begins with 'r-' followed by from 4 to 32 lowercase letters or digits. * Organizational unit (OU) - A string that begins with 'ou-' followed by from 4 to 32 lowercase letters or digits (the ID of the root that the OU is in). This string is followed by a second '-' dash and from 8 to 32 additional lowercase letters or digits. Regex Pattern: '^(r-[0-9a-z]{4,32})|(ou-[0-9a-z]{4,32}-[a-z0-9]{8,32})$'",
						Required:            true,
						Optional:            false,
						Computed:            false,
					},
					...
					"tags": schema.ListNestedAttribute{
						...
						MarkdownDescription: "A list of tags that you want to attach to the newly created OU. For each tag in the list, you must specify both a tag key and a value. You can set the value to an empty string, but you can't set it to null. For more information about tagging, see Tagging Organizations resources (https://docs.aws.amazon.com/organizations/latest/userguide/orgs_tagging.html) in the Organizations User Guide. If any one of the tags is not valid or if you exceed the allowed number of tags for an OU, then the entire request fails and the OU is not created.",
