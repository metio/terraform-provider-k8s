...
var (
	_ datasource.DataSource = &ControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1Manifest{}
)

func NewControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1Manifest() datasource.DataSource {
	return &ControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1Manifest{}
}

type ControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1Manifest struct{}

type ControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1ManifestData struct {
	...
	Spec *struct {
		...
		TargetEndpoint *string `tfsdk:"target_endpoint" json:"targetEndpoint,omitempty"`
...
func (r *ControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1Manifest) Metadata(_ context.Context, request datasource.MetadataRequest, response *datasource.MetadataResponse) {
	response.TypeName = request.ProviderTypeName + "_controlplane_operator_openshift_io_pod_network_connectivity_check_v1alpha1_manifest"
}
...
func (r *ControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1Manifest) Schema(_ context.Context, _ datasource.SchemaRequest, response *datasource.SchemaResponse) {
	response.Schema = schema.Schema{
		...
		MarkdownDescription: "PodNetworkConnectivityCheck  Compatibility level 4: No compatibility is provided, the API can change at any point for any reason. These capabilities should not be used by applications needing long term support.",
		Attributes: map[string]schema.Attribute{
			...
			"spec": schema.SingleNestedAttribute{
				...
				MarkdownDescription: "Spec defines the source and target of the connectivity check",
				Attributes: map[string]schema.Attribute{
					"source_pod": schema.StringAttribute{
						Description:         "SourcePod names the pod from which the condition will be checked",
						MarkdownDescription: "SourcePod names the pod from which the condition will be checked",
						Required:            true,
						Optional:            false,
						Computed:            false,
						Validators: []validator.String{
					...
					"target_endpoint": schema.StringAttribute{
						...
						MarkdownDescription: "EndpointAddress to check. A TCP address of the form host:port. Note that if host is a DNS name, then the check would fail if the DNS name cannot be resolved. Specify an IP address for host to bypass DNS name lookup.",
						Required:            true,
						Optional:            false,
						Computed:            false,
						Validators: []validator.String{
							stringvalidator.RegexMatches(regexp.MustCompile(`^\S+:\d*$`), ""),
						},
					},
					...
					"tls_client_cert": schema.SingleNestedAttribute{
						...
						MarkdownDescription: "TLSClientCert, if specified, references a kubernetes.io/tls type secret with 'tls.crt' and 'tls.key' entries containing an optional TLS client certificate and key to be used when checking endpoints that require a client certificate in order to gracefully preform the scan without causing excessive logging in the endpoint process. The secret must exist in the same namespace as this resource.",
						Attributes: map[string]schema.Attribute{
							"name": schema.StringAttribute{
...
func (r *ControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1Manifest) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	tflog.Debug(ctx, "Read resource k8s_controlplane_operator_openshift_io_pod_network_connectivity_check_v1alpha1_manifest")

	var model ControlplaneOperatorOpenshiftIoPodNetworkConnectivityCheckV1Alpha1ManifestData
	...
	model.ApiVersion = pointer.String("controlplane.operator.openshift.io/v1alpha1")
	model.Kind = pointer.String("PodNetworkConnectivityCheck")
