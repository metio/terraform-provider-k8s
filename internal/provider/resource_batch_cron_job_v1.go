...
type BatchCronJobV1Resource struct{}

var (
	_ resource.Resource = (*BatchCronJobV1Resource)(nil)
)

type BatchCronJobV1TerraformModel struct {
...
type BatchCronJobV1GoModel struct {
	...
	Spec *struct {
		...
		JobTemplate *struct {
			...
			Spec *struct {
				...
				CompletionMode *string `tfsdk:"completion_mode" yaml:"completionMode,omitempty"`

				Completions *int64 `tfsdk:"completions" yaml:"completions,omitempty"`

				ManualSelector *bool `tfsdk:"manual_selector" yaml:"manualSelector,omitempty"`

				Parallelism *int64 `tfsdk:"parallelism" yaml:"parallelism,omitempty"`
				...
				PodFailurePolicy *struct {
					Rules *[]struct {
						...
						} `tfsdk:"on_exit_codes" yaml:"onExitCodes,omitempty"`

						OnPodConditions *[]struct {
							Status *string `tfsdk:"status" yaml:"status,omitempty"`

							Type *string `tfsdk:"type" yaml:"type,omitempty"`
						} `tfsdk:"on_pod_conditions" yaml:"onPodConditions,omitempty"`
					} `tfsdk:"rules" yaml:"rules,omitempty"`
				} `tfsdk:"pod_failure_policy" yaml:"podFailurePolicy,omitempty"`
		...
		} `tfsdk:"job_template" yaml:"jobTemplate,omitempty"`
...
func NewBatchCronJobV1Resource() resource.Resource {
	return &BatchCronJobV1Resource{}
}

func (r *BatchCronJobV1Resource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_batch_cron_job_v1"
}

func (r *BatchCronJobV1Resource) GetSchema(_ context.Context) (tfsdk.Schema, diag.Diagnostics) {
	return tfsdk.Schema{
		Description:         "CronJob represents the configuration of a single cron job.",
		MarkdownDescription: "CronJob represents the configuration of a single cron job.",
		Attributes: map[string]tfsdk.Attribute{
			...
			"spec": {
				...
				MarkdownDescription: "CronJobSpec describes how the job execution will look like and when it will actually run.",

				Attributes: tfsdk.SingleNestedAttributes(map[string]tfsdk.Attribute{

					"concurrency_policy": {
						Description:         "Specifies how to treat concurrent executions of a Job. Valid values are: - 'Allow' (default): allows CronJobs to run concurrently; - 'Forbid': forbids concurrent runs, skipping next run if previous run hasn't finished yet; - 'Replace': cancels currently running job and replaces it with a new one",
						MarkdownDescription: "Specifies how to treat concurrent executions of a Job. Valid values are: - 'Allow' (default): allows CronJobs to run concurrently; - 'Forbid': forbids concurrent runs, skipping next run if previous run hasn't finished yet; - 'Replace': cancels currently running job and replaces it with a new one",
					...
					"job_template": {
						...
						MarkdownDescription: "JobTemplateSpec describes the data a Job should have when created from a template",
...
func (r *BatchCronJobV1Resource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Create resource k8s_batch_cron_job_v1")

	var state BatchCronJobV1TerraformModel
	diags := req.Config.Get(ctx, &state)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	var goModel BatchCronJobV1GoModel
	...
	goModel.ApiVersion = utilities.Ptr("batch/v1")
	goModel.Kind = utilities.Ptr("CronJob")
...
func (r *BatchCronJobV1Resource) Read(ctx context.Context, _ resource.ReadRequest, _ *resource.ReadResponse) {
	tflog.Debug(ctx, "Read resource k8s_batch_cron_job_v1")
	// NO-OP: All data is already in Terraform state
}

func (r *BatchCronJobV1Resource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	tflog.Debug(ctx, "Update resource k8s_batch_cron_job_v1")
...
func (r *BatchCronJobV1Resource) Delete(ctx context.Context, _ resource.DeleteRequest, _ *resource.DeleteResponse) {
	tflog.Debug(ctx, "Delete resource k8s_batch_cron_job_v1")
