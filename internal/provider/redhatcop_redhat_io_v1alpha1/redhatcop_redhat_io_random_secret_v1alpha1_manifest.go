...
var (
	_ datasource.DataSource = &RedhatcopRedhatIoRandomSecretV1Alpha1Manifest{}
)

func NewRedhatcopRedhatIoRandomSecretV1Alpha1Manifest() datasource.DataSource {
	return &RedhatcopRedhatIoRandomSecretV1Alpha1Manifest{}
}

type RedhatcopRedhatIoRandomSecretV1Alpha1Manifest struct{}

type RedhatcopRedhatIoRandomSecretV1Alpha1ManifestData struct {
	...
	Spec *struct {
		...
		IsKVSecretsEngineV2  *bool   `tfsdk:"is_kv_secrets_engine_v2" json:"isKVSecretsEngineV2,omitempty"`
		KvSecretRetainPolicy *string `tfsdk:"kv_secret_retain_policy" json:"kvSecretRetainPolicy,omitempty"`
		Name                 *string `tfsdk:"name" json:"name,omitempty"`
		Path                 *string `tfsdk:"path" json:"path,omitempty"`
		RefreshPeriod        *string `tfsdk:"refresh_period" json:"refreshPeriod,omitempty"`
		SecretFormat         *struct {
			InlinePasswordPolicy *string `tfsdk:"inline_password_policy" json:"inlinePasswordPolicy,omitempty"`
			PasswordPolicyName   *string `tfsdk:"password_policy_name" json:"passwordPolicyName,omitempty"`
		} `tfsdk:"secret_format" json:"secretFormat,omitempty"`
		SecretKey *string `tfsdk:"secret_key" json:"secretKey,omitempty"`
	} `tfsdk:"spec" json:"spec,omitempty"`
}

func (r *RedhatcopRedhatIoRandomSecretV1Alpha1Manifest) Metadata(_ context.Context, request datasource.MetadataRequest, response *datasource.MetadataResponse) {
	response.TypeName = request.ProviderTypeName + "_redhatcop_redhat_io_random_secret_v1alpha1_manifest"
}

func (r *RedhatcopRedhatIoRandomSecretV1Alpha1Manifest) Schema(_ context.Context, _ datasource.SchemaRequest, response *datasource.SchemaResponse) {
	response.Schema = schema.Schema{
		Description:         "RandomSecret is the Schema for the randomsecrets API",
		MarkdownDescription: "RandomSecret is the Schema for the randomsecrets API",
		Attributes: map[string]schema.Attribute{
			...
			"spec": schema.SingleNestedAttribute{
				...
				MarkdownDescription: "RandomSecretSpec defines the desired state of RandomSecret",
				Attributes: map[string]schema.Attribute{
					...
					"is_kv_secrets_engine_v2": schema.BoolAttribute{
						...
						MarkdownDescription: "IsKVSecretsEngineV2 indicates if the KV Secrets engine is V2 or not. Default is false to indicate the payload to send is for KV Secret Engine V1.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},

					"kv_secret_retain_policy": schema.StringAttribute{
						Description:         "The KV secret retain policy to apply when the Kubernetes resource is deleted. When unspecified, the KV secret is also deleted.",
						MarkdownDescription: "The KV secret retain policy to apply when the Kubernetes resource is deleted. When unspecified, the KV secret is also deleted.",
						Required:            false,
						Optional:            true,
						Computed:            false,
						Validators: []validator.String{
					...
					"refresh_period": schema.StringAttribute{
						...
						MarkdownDescription: "RefreshPeriod if specified, the operator will refresh the secret with the given frequency. This will also set the ttl of the secret which provides a hint for how often consumers should check back for a new value when reading the secret's lease_duration.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},

					"secret_format": schema.SingleNestedAttribute{
						Description:         "SecretFormat specifies a map of key and password policies used to generate random values",
						MarkdownDescription: "SecretFormat specifies a map of key and password policies used to generate random values",
						Attributes: map[string]schema.Attribute{
							"inline_password_policy": schema.StringAttribute{
								Description:         "InlinePasswordPolicy is an inline password policy specified using Vault password policy syntax (https://www.vaultproject.io/docs/concepts/password-policies#password-policy-syntax) Only one of PasswordPolicyName or InlinePasswordPolicy can be specified",
								MarkdownDescription: "InlinePasswordPolicy is an inline password policy specified using Vault password policy syntax (https://www.vaultproject.io/docs/concepts/password-policies#password-policy-syntax) Only one of PasswordPolicyName or InlinePasswordPolicy can be specified",
								Required:            false,
								Optional:            true,
								Computed:            false,
							},

							"password_policy_name": schema.StringAttribute{
								Description:         "PasswordPolicyName a ref to a password policy defined in Vault. Notice that in order to use this, the Vault role you use needs the following capabilities = ['read'] on /sys/policy/password. Only one of PasswordPolicyName or InlinePasswordPolicy can be specified",
								MarkdownDescription: "PasswordPolicyName a ref to a password policy defined in Vault. Notice that in order to use this, the Vault role you use needs the following capabilities = ['read'] on /sys/policy/password. Only one of PasswordPolicyName or InlinePasswordPolicy can be specified",
								Required:            false,
								Optional:            true,
								Computed:            false,
							},
					...
					"secret_key": schema.StringAttribute{
						...
						MarkdownDescription: "SecretKey is the key to be used for this secret when stored in Vault kv",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},
...
func (r *RedhatcopRedhatIoRandomSecretV1Alpha1Manifest) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	tflog.Debug(ctx, "Read resource k8s_redhatcop_redhat_io_random_secret_v1alpha1_manifest")

	var model RedhatcopRedhatIoRandomSecretV1Alpha1ManifestData
	...
	model.Kind = pointer.String("RandomSecret")
