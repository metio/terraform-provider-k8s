...
var (
	_ datasource.DataSource = &MonitoringCoreosComThanosRulerV1Manifest{}
)

func NewMonitoringCoreosComThanosRulerV1Manifest() datasource.DataSource {
	return &MonitoringCoreosComThanosRulerV1Manifest{}
}

type MonitoringCoreosComThanosRulerV1Manifest struct{}

type MonitoringCoreosComThanosRulerV1ManifestData struct {
...
func (r *MonitoringCoreosComThanosRulerV1Manifest) Metadata(_ context.Context, request datasource.MetadataRequest, response *datasource.MetadataResponse) {
	response.TypeName = request.ProviderTypeName + "_monitoring_coreos_com_thanos_ruler_v1_manifest"
}
			...
			"spec": schema.SingleNestedAttribute{
				...
				Attributes: map[string]schema.Attribute{
					"additional_args": schema.ListNestedAttribute{
						Description:         "AdditionalArgs allows setting additional arguments for the ThanosRuler container.It is intended for e.g. activating hidden flags which are not supported bythe dedicated configuration options yet. The arguments are passed as-is to theThanosRuler container which may cause issues if they are invalid or not supportedby the given ThanosRuler version.In case of an argument conflict (e.g. an argument which is already set by theoperator itself) or when providing an invalid argument the reconciliation willfail and an error will be logged.",
					...
					"alert_query_url": schema.StringAttribute{
						Description:         "The external Query URL the Thanos Ruler will set in the 'Source' fieldof all alerts.Maps to the '--alert.query-url' CLI arg.",
						MarkdownDescription: "The external Query URL the Thanos Ruler will set in the 'Source' fieldof all alerts.Maps to the '--alert.query-url' CLI arg.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},
					...
					"alert_relabel_configs": schema.SingleNestedAttribute{
						Description:         "AlertRelabelConfigs configures alert relabeling in ThanosRuler.Alert relabel configurations must have the form as specified in the official Prometheus documentation:https://prometheus.io/docs/prometheus/latest/configuration/configuration/#alert_relabel_configsAlternative to AlertRelabelConfigFile, and lower order priority.",
					...
					"alertmanagers_config": schema.SingleNestedAttribute{
						Description:         "Define configuration for connecting to alertmanager.  Only available with thanos v0.10.0and higher.  Maps to the 'alertmanagers.config' arg.",
						MarkdownDescription: "Define configuration for connecting to alertmanager.  Only available with thanos v0.10.0and higher.  Maps to the 'alertmanagers.config' arg.",
						Attributes: map[string]schema.Attribute{
							"key": schema.StringAttribute{
								Description:         "The key of the secret to select from.  Must be a valid secret key.",
								MarkdownDescription: "The key of the secret to select from.  Must be a valid secret key.",
					...
					"alertmanagers_url": schema.ListAttribute{
						Description:         "Define URLs to send alerts to Alertmanager.  For Thanos v0.10.0 and higher,AlertManagersConfig should be used instead.  Note: this field will be ignoredif AlertManagersConfig is specified.Maps to the 'alertmanagers.url' arg.",
						MarkdownDescription: "Define URLs to send alerts to Alertmanager.  For Thanos v0.10.0 and higher,AlertManagersConfig should be used instead.  Note: this field will be ignoredif AlertManagersConfig is specified.Maps to the 'alertmanagers.url' arg.",
						ElementType:         types.StringType,
						Required:            false,
						Optional:            true,
						Computed:            false,
					...
					"containers": schema.ListNestedAttribute{
						Description:         "Containers allows injecting additional containers or modifying operator generatedcontainers. This can be used to allow adding an authentication proxy to a ThanosRuler pod orto change the behavior of an operator generated container. Containers described here modifyan operator generated container if they share the same name and modifications are done via astrategic merge patch. The current container names are: 'thanos-ruler' and 'config-reloader'.Overriding containers is entirely outside the scope of what the maintainers will support and by doingso, you accept that this behaviour may break at any time without notice.",
					...
					"excluded_from_enforcement": schema.ListNestedAttribute{
						Description:         "List of references to PrometheusRule objectsto be excluded from enforcing a namespace label of origin.Applies only if enforcedNamespaceLabel set to true.",
						MarkdownDescription: "List of references to PrometheusRule objectsto be excluded from enforcing a namespace label of origin.Applies only if enforcedNamespaceLabel set to true.",
						NestedObject: schema.NestedAttributeObject{
							Attributes: map[string]schema.Attribute{
					...
					"external_prefix": schema.StringAttribute{
						Description:         "The external URL the Thanos Ruler instances will be available under. This isnecessary to generate correct URLs. This is necessary if Thanos Ruler is notserved from root of a DNS name.",
						MarkdownDescription: "The external URL the Thanos Ruler instances will be available under. This isnecessary to generate correct URLs. This is necessary if Thanos Ruler is notserved from root of a DNS name.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},
					...
					"grpc_server_tls_config": schema.SingleNestedAttribute{
						Description:         "GRPCServerTLSConfig configures the gRPC server from which Thanos Querier readsrecorded rule data.Note: Currently only the CAFile, CertFile, and KeyFile fields are supported.Maps to the '--grpc-server-tls-*' CLI args.",
						MarkdownDescription: "GRPCServerTLSConfig configures the gRPC server from which Thanos Querier readsrecorded rule data.Note: Currently only the CAFile, CertFile, and KeyFile fields are supported.Maps to the '--grpc-server-tls-*' CLI args.",
						Attributes: map[string]schema.Attribute{
					...
					"image_pull_secrets": schema.ListNestedAttribute{
						Description:         "An optional list of references to secrets in the same namespaceto use for pulling thanos images from registriessee http://kubernetes.io/docs/user-guide/images#specifying-imagepullsecrets-on-a-pod",
						MarkdownDescription: "An optional list of references to secrets in the same namespaceto use for pulling thanos images from registriessee http://kubernetes.io/docs/user-guide/images#specifying-imagepullsecrets-on-a-pod",
						NestedObject: schema.NestedAttributeObject{
							Attributes: map[string]schema.Attribute{
								"name": schema.StringAttribute{
									Description:         "Name of the referent.This field is effectively required, but due to backwards compatibility isallowed to be empty. Instances of this type with an empty value here arealmost certainly wrong.More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/names/#names",
					...
					"listen_local": schema.BoolAttribute{
						Description:         "ListenLocal makes the Thanos ruler listen on loopback, so that itdoes not bind against the Pod IP.",
						MarkdownDescription: "ListenLocal makes the Thanos ruler listen on loopback, so that itdoes not bind against the Pod IP.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},
					...
					"paused": schema.BoolAttribute{
						Description:         "When a ThanosRuler deployment is paused, no actions except for deletionwill be performed on the underlying objects.",
						MarkdownDescription: "When a ThanosRuler deployment is paused, no actions except for deletionwill be performed on the underlying objects.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},
					...
					"prometheus_rules_excluded_from_enforce": schema.ListNestedAttribute{
						Description:         "PrometheusRulesExcludedFromEnforce - list of Prometheus rules to be excluded from enforcingof adding namespace labels. Works only if enforcedNamespaceLabel set to true.Make sure both ruleNamespace and ruleName are set for each pairDeprecated: use excludedFromEnforcement instead.",
						MarkdownDescription: "PrometheusRulesExcludedFromEnforce - list of Prometheus rules to be excluded from enforcingof adding namespace labels. Works only if enforcedNamespaceLabel set to true.Make sure both ruleNamespace and ruleName are set for each pairDeprecated: use excludedFromEnforcement instead.",
						NestedObject: schema.NestedAttributeObject{
							Attributes: map[string]schema.Attribute{
					...
					"rule_namespace_selector": schema.SingleNestedAttribute{
						Description:         "Namespaces to be selected for Rules discovery. If unspecified, onlythe same namespace as the ThanosRuler object is in is used.",
						MarkdownDescription: "Namespaces to be selected for Rules discovery. If unspecified, onlythe same namespace as the ThanosRuler object is in is used.",
						Attributes: map[string]schema.Attribute{
							"match_expressions": schema.ListNestedAttribute{
								Description:         "matchExpressions is a list of label selector requirements. The requirements are ANDed.",
								MarkdownDescription: "matchExpressions is a list of label selector requirements. The requirements are ANDed.",
					...
					"rule_selector": schema.SingleNestedAttribute{
						Description:         "A label selector to select which PrometheusRules to mount for alerting andrecording.",
						MarkdownDescription: "A label selector to select which PrometheusRules to mount for alerting andrecording.",
						Attributes: map[string]schema.Attribute{
							"match_expressions": schema.ListNestedAttribute{
								Description:         "matchExpressions is a list of label selector requirements. The requirements are ANDed.",
								MarkdownDescription: "matchExpressions is a list of label selector requirements. The requirements are ANDed.",
					...
					"service_account_name": schema.StringAttribute{
						Description:         "ServiceAccountName is the name of the ServiceAccount to use to run theThanos Ruler Pods.",
						MarkdownDescription: "ServiceAccountName is the name of the ServiceAccount to use to run theThanos Ruler Pods.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},
					...
					"tracing_config": schema.SingleNestedAttribute{
						Description:         "TracingConfig configures tracing in Thanos.'tracingConfigFile' takes precedence over this field.This is an *experimental feature*, it may change in any upcoming releasein a breaking way.",
						MarkdownDescription: "TracingConfig configures tracing in Thanos.'tracingConfigFile' takes precedence over this field.This is an *experimental feature*, it may change in any upcoming releasein a breaking way.",
						Attributes: map[string]schema.Attribute{
							"key": schema.StringAttribute{
								Description:         "The key of the secret to select from.  Must be a valid secret key.",
								MarkdownDescription: "The key of the secret to select from.  Must be a valid secret key.",
					...
					"tracing_config_file": schema.StringAttribute{
						Description:         "TracingConfig specifies the path of the tracing configuration file.This field takes precedence over 'tracingConfig'.This is an *experimental feature*, it may change in any upcoming releasein a breaking way.",
						MarkdownDescription: "TracingConfig specifies the path of the tracing configuration file.This field takes precedence over 'tracingConfig'.This is an *experimental feature*, it may change in any upcoming releasein a breaking way.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},
					...
					"volumes": schema.ListNestedAttribute{
						Description:         "Volumes allows configuration of additional volumes on the output StatefulSet definition. Volumes specified willbe appended to other volumes that are generated as a result of StorageSpec objects.",
						MarkdownDescription: "Volumes allows configuration of additional volumes on the output StatefulSet definition. Volumes specified willbe appended to other volumes that are generated as a result of StorageSpec objects.",
						NestedObject: schema.NestedAttributeObject{
							Attributes: map[string]schema.Attribute{
					...
					"web": schema.SingleNestedAttribute{
						...
						MarkdownDescription: "Defines the configuration of the ThanosRuler web server.",
						Attributes: map[string]schema.Attribute{
...
func (r *MonitoringCoreosComThanosRulerV1Manifest) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	tflog.Debug(ctx, "Read resource k8s_monitoring_coreos_com_thanos_ruler_v1_manifest")

	var model MonitoringCoreosComThanosRulerV1ManifestData
