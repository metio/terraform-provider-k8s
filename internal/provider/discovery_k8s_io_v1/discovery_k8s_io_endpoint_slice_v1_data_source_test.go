...
func TestDiscoveryK8SIoEndpointSliceV1DataSource_ConfigurationErrors(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			testutilities.VerifyConfigurationErrors(t, "data", "k8s_discovery_k8s_io_endpoint_slice_v1", testCase)
		})
	}
}

func TestDiscoveryK8SIoEndpointSliceV1DataSource_OfflineUsage(t *testing.T) {
	configuration := `
		metadata = {
			name = "some"
			namespace = "somewhere"
	...
	`
	testutilities.VerifyCannotBeUsedOffline(t, "data", "k8s_discovery_k8s_io_endpoint_slice_v1", configuration)
}
