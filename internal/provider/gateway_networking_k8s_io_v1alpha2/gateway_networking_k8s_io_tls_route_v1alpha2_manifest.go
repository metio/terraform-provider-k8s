...
type GatewayNetworkingK8SIoTlsrouteV1Alpha2ManifestData struct {
	...
	Spec *struct {
		...
		UseDefaultGateways *string `tfsdk:"use_default_gateways" json:"useDefaultGateways,omitempty"`
		...
		Attributes: map[string]schema.Attribute{
			...
			"spec": schema.SingleNestedAttribute{
				...
				Attributes: map[string]schema.Attribute{
					...
					"use_default_gateways": schema.StringAttribute{
						...
						MarkdownDescription: "UseDefaultGateways indicates the default Gateway scope to use for this Route. If unset (the default) or set to None, the Route will not be attached to any default Gateway; if set, it will be attached to any default Gateway supporting the named scope, subject to the usual rules about which Routes a Gateway is allowed to claim. Think carefully before using this functionality! The set of default Gateways supporting the requested scope can change over time without any notice to the Route author, and in many situations it will not be appropriate to request a default Gateway for a given Route -- for example, a Route with specific security requirements should almost certainly not use a default Gateway.",
						Required:            false,
						Optional:            true,
						Computed:            false,
						Validators: []validator.String{
							stringvalidator.OneOf("All", "None"),
						},
					},
				},
				Required: true,
