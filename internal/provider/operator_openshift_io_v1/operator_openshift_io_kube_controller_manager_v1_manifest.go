...
var (
	_ datasource.DataSource = &OperatorOpenshiftIoKubeControllerManagerV1Manifest{}
)

func NewOperatorOpenshiftIoKubeControllerManagerV1Manifest() datasource.DataSource {
	return &OperatorOpenshiftIoKubeControllerManagerV1Manifest{}
}
...
func (r *OperatorOpenshiftIoKubeControllerManagerV1Manifest) Metadata(_ context.Context, request datasource.MetadataRequest, response *datasource.MetadataResponse) {
	response.TypeName = request.ProviderTypeName + "_operator_openshift_io_kube_controller_manager_v1_manifest"
}

func (r *OperatorOpenshiftIoKubeControllerManagerV1Manifest) Schema(_ context.Context, _ datasource.SchemaRequest, response *datasource.SchemaResponse) {
	response.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"spec": schema.SingleNestedAttribute{
				...
				MarkdownDescription: "spec is the specification of the desired behavior of the Kubernetes Controller Manager",
				Attributes: map[string]schema.Attribute{
					...
					"use_more_secure_service_ca": schema.BoolAttribute{
						...
						MarkdownDescription: "useMoreSecureServiceCA indicates that the service-ca.crt provided in SA token volumes should include only enough certificates to validate service serving certificates. Once set to true, it cannot be set to false. Even if someone finds a way to set it back to false, the service-ca.crt files that previously existed will only have the more secure content.",
						Required:            false,
						Optional:            true,
						Computed:            false,
					},
...
func (r *OperatorOpenshiftIoKubeControllerManagerV1Manifest) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	tflog.Debug(ctx, "Read resource k8s_operator_openshift_io_kube_controller_manager_v1_manifest")

	var model OperatorOpenshiftIoKubeControllerManagerV1ManifestData
