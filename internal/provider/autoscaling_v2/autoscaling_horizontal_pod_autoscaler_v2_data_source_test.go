...
func TestAutoscalingHorizontalPodAutoscalerV2DataSource_ConfigurationErrors(t *testing.T) {
	...
	for name, testCase := range testCases {
		t.Run(name, func(t *testing.T) {
			testutilities.VerifyConfigurationErrors(t, "data", "k8s_autoscaling_horizontal_pod_autoscaler_v2", testCase)
		})
	}
}

func TestAutoscalingHorizontalPodAutoscalerV2DataSource_OfflineUsage(t *testing.T) {
	configuration := `
		metadata = {
			name = "some"
			namespace = "somewhere"
	...
	`
	testutilities.VerifyCannotBeUsedOffline(t, "data", "k8s_autoscaling_horizontal_pod_autoscaler_v2", configuration)
}
